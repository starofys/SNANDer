...
static int ch347_spi_send_command(unsigned int writecnt, unsigned int readcnt, const unsigned char *writearr, unsigned char *readarr)
{
	int ret;
	// ch347_cs_control(CH347_CS_ASSERT | CH347_CS_CHANGE, CH347_CS_IGNORE);
	if (writecnt) {
...
static int ch347_spi_shutdown(void)
{
#ifdef _WIN32
...
static int ch347_spi_init(void)
{
    ...
    if (open_res == -1){
        DevIsOpened = FALSE;
        printf("Couldn't open CH347T(%x:%x)/CH347F(%x:%x) device. \n",
        	devs_ch347_spi[0].vendor_id,devs_ch347_spi[0].device_id,
        	devs_ch347_spi[1].vendor_id,devs_ch347_spi[1].device_id);
		return -1;
    }else {
        DevIsOpened = TRUE;
