...
int i2c_eeprom_read(unsigned char *buf, unsigned long from, unsigned long len)
{
	...
	if (device_opt->read_eeprom(pbuf, eepromsize, &eeprom_info) < 0) {
...
int i2c_eeprom_erase(unsigned long offs, unsigned long len)
{
	...
	if(device_opt->write_eeprom(pbuf, eepromsize, &eeprom_info) < 0) {
...
long i2c_init(void)
{
	if (device_opt->config_stream(CH341_I2C_STANDARD_SPEED) < 0)
		return -1;
